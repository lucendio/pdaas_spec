...
pdf:
	...
	\
	--template="$(TEMPLATES_DIR)/spec.pdf.template" \
	\
	...
	\
	--include-in-header=$(STYLES_DIR)/spec.latex-header.tex \
	\
	"$(CWD)/metadata.yml" \
	"$(STYLES_DIR)/spec.variables.yml" \
	"$(CONTENTS_DIR)/"*.md
...
html:
	...
	--filter=pandoc-citeproc \
	\
	--template="$(TEMPLATES_DIR)/spec.web.template" \
	\
	--css="./assets/styles/spec-md.css" \
	--section-divs \
	--jsmath \
	\
	--to=html5 \
